...
> **Date :** 2026-07-23 (soir) · **Statut :** carte de l'état actuel, à maintenir (pas un historique).
...
- **Social** : `social-webhook.js` (live/shadow, dédup `social_bot_log`), `social-poll.js` (sans webhook, shadow), `social-draft.js` (veille groupes FB), `social.js` (publication **FB + IG + YouTube Shorts** des drafts éditoriaux — canal `yt` ajouté 2026-07-23, délègue à `youtube-upload.js`, **opt-in `YOUTUBE_AUTOPUBLISH`**). Mode **shadow par défaut**.
- **YouTube** (2026-07-23, `ADR-YT-001`) : `youtube-upload.js` — upload resumable de Shorts (réutilise le MP4 vertical des Reels), OAuth provider `youtube` (`_googleOAuth.js`, scopes `youtube.upload` + `youtube.force-ssl`). `privacyStatus` **privé par DÉFAUT** (public explicite uniquement). `GET ?status=1` = connexion + `capabilities[]` + `autopublish` en LECTURE SEULE ; `DELETE ?videoId=` = réparation.
- **Meta Ads (payant) & agent budget pub** (2026-07-19, advisory strict — jamais de dépense déclenchée par le code) : `meta-ads-campaign.js` crée les campagnes/ad sets/créatives/annonces depuis `src/config/metaCampaignBrief.js` (`src/utils/metaCampaignBuilder.js`) — **toujours `status:PAUSED`**, carrousels multi-photos (`child_attachments`, `refreshCreatives:true` pour remplacer), idempotent. `meta-ads-insights.js` (`src/utils/metaAdsInsights.js`) = **brique MESURE** : lit spend/conversions/ROAS Meta par campagne/ad set + bloc `health` (`no_spend`/`traffic_only`/`ok`) qui interdit d'arbitrer sur un ROAS non tracké. `ad-budget-agent.js` (`src/utils/adBudgetAgent.js`) = **brique DÉCISION advisory** : **vision budget par bien** (CAC plafond = 50% commission Booking nette évitée, RM-08 — seuls Amaryllis/Zandoli/Géko éligibles) + arbitrage scale/hold/cut. `ad-budget-execute.js` = **brique EXÉCUTION** (2026-07-19) : pause/baisse -25%/hausse +20% d'ad sets Meta actifs, **plafond GLOBAL DUR 600€/mois** (`enforceGlobalMonthlyCap`), garde-fous STRUCTURELS — les seules primitives d'écriture Meta = `pauseAdSet`/`setAdSetBudgetCents` (dans `meta-ads-campaign.js`), **aucune** ne peut activer un ad set en pause ni créer une campagne (règle absolue par construction, même en `live`). Mode `shadow` par défaut (`AD_AGENT_MODE`), kill-switch `AD_AGENT_DISABLED`, trace D1 `ad_budget_actions`, `attribution_survey` (questionnaire post-achat, niveau 3 de la mesure pub), cron 9h. `meta-ads-campaign.js` a aussi des outils : `?debug=status` (effective_status campagne+adsets+ads), `?debug=list` (toutes les campagnes), `POST action=prune` (supprime ad sets hors-brief), et **`POST action=swapCreativeLink`** (2026-07-21 — change le LIEN d'une annonce active sans la pauser, contrairement à `refreshCreatives` qui repause ; pousse les UTM sans coupure). **Depuis 2026-07-21 : `AD_AGENT_MODE=live`** (l'agent agit réellement dans ses plafonds). **Meta insights lus via `time_range` explicite** (pas `date_preset`, qui sert des données périmées sur ce compte). Google Ads = **lecture seule intégrée au MÊME moteur de décision** que Meta (`_googleAds.js`/`google-ads-insights.js`, API v21, OAuth `googleads` — aucune primitive `mutate`) : `bienIdFromGoogleCampaignName` mappe les campagnes au CAC RM-08 ; les campagnes audience multi-biens (ex. "Canada") ont un plafond "pool" (`GOOGLE_CAMPAIGN_POOLS`, moyenne des biens éligibles). `ADR-AD-BUDGET-AGENT-001`, `ADR-AD-BUDGET-EXECUTE-001`, `ADR-META-CARROUSELS-001`, `ADR-GOOGLE-BUDGET-UNIFIED-001`, `ADR-META-INSIGHTS-TIMERANGE-001`, `ADR-META-UTM-SWAP-001`, `ADR-AD-AGENT-LIVE-001`. **MESURE EN 3 NIVEAUX depuis 2026-07-23** (`ADR-MESURE-PUB-001`) : `ad-budget-agent.js` renvoie `niveau1_plateforme` (ROAS/CTR/CPM, explicitement `decision_business:false` — pilotage quotidien A vs B seulement) · `niveau2_finance` (`src/utils/adFinanceKpis.js` — MER d'acquisition, **CAC blended** (jamais par canal : ça supposerait une attribution fiable), LTV, LTV/CAC, lus dans D1 `direct_bookings` ; un MER < 1 n'est jamais racheté par un LTV/CAC flatteur → verdict `payback_differe`) · `niveau3_declaratif` (`attribution-survey.js` + question 1 clic sur `/merci`, table D1 `attribution_survey`, ventilation marquée `exploitable:false` sous 20 réponses). Métriques niveau 1 assainies dans `src/utils/metaAdsInsights.js` : couverture unique (`reach`) au lieu des impressions, **CPMR** (=CPM×répétition, détecte la saturation) au lieu du CPM, **CPC sortant** (`outbound_clicks`) au lieu du CTR, **attribution `7d_click` seule** (fin du crédit sur simple vue) ; l'agrégat expose `reachSum`/`cpmrFloor`/`reachIsDeduplicated:false` car la couverture N'EST PAS additive. `src/utils/audienceSaturation.js` (⚠️ commit `b02e774` **non déployé**, CI bloquée) audite la **répétition par segment d'audience** (`?breakdown=audience` → `user_segment_key`, **nom de breakdown non encore vérifié sur le compte réel**) : au-delà de 10 expositions/30 j sur un segment qui connaît déjà la marque, budget récupérable → baisse chiffrée, annoncée comme estimation linéaire. `src/utils/promoCost.js` (commit `cecadcb`, non déployé) chiffre le vrai coût d'une remise (compression = remise ÷ marge + 2 coûts non chiffrables). `ADR-MESURE-PUB-001`, `ADR-PROMO-001`. `social.js` expose `POST action=delete_post` (supprime un post FB/IG, nettoyage de doublon). Page annexe `functions/depart-tardif.js` (page de paiement autonome late check-out, hors routing SPA).
...
- **Rotation planner** (`editorial-calendar.js`, `seed_30days`) : séquence stable de 6 biens (Iguana exclu, bail long) indexée sur `Math.floor(scheduled_at/86400) % 6` — invariant : à 1 post/jour, un bien ne revient jamais à <4j d'intervalle (respecte l'anti-doublon du gate). L'ancienne séquence pondérée (Amaryllis 8×/30j) créait des écarts de 2-3j → escalades systématiques → soirées sans post (incidents fin juin/début juillet).
  ...
  - **Fermeture autonome du backlog vérifiable** (`backlog-verify.js`, I-11, 2026-07-18, cron lundi 6h UTC) : distinct du triage ci-dessus (qui BLOQUE des items hors-sol) — celui-ci FERME des items dont la revendication est un fait technique numériquement vérifiable. LLM classifie chaque item `backlog`/`a-planifier` (catégories seo/tracking/content/technique/performance) en `{checkable,checkType,params}` → `applyKeywordGuard()` (`src/utils/backlogVerify.js`) rétrograde en `checkable:false` toute classification sans appui textuel réel dans l'item (garde ajoutée après un faux positif vu en dry-run prod : "Core Web Vitals" mappé à tort sur une vérif meta SEO sans rapport) → checker déterministe (jamais le LLM) : `live_meta` (title/meta live), `ga4_event` (`_ga4.js`, event vu sur 30j), `jsonld_schema` (grep `@type` JSON-LD live). Seul un positif ferme (`agent_actions.status='fait'`, note `🤖 Auto-vérifié`). N'écrit JAMAIS dans `action_outcomes` (préserverait la véracité de `delegation-stats.action_cochee`, qui mesure la charge de VINCENT, pas celle de la machine) — traçabilité propre via `backlog_autoverify_log`. Backlog actuel à majorité contenu physique/créatif (photos/Reels) → 0 fermeture au lancement, honnête (pas un bug) ; valeur prospective au fil des items créés par la fleet.
...
- **Auto-publication réseaux LIVE** : drafts J-2 → gate → posts FB+IG (`runEditorialAutoPublish`, cron `*/10`, 1 post/2h → `agent-drafts publish` → `social.js`). Fact-check de dernière minute (fail-OPEN à la publication, contrairement au gate fail-CLOSED). Fenêtre 90 jours en arrière. **⚠️ Invariants anti-blocage/anti-doublon (2026-07-22, `ADR-SOCIAL-AUTOPUB-INCIDENT-001`)** : (a) l'auto-publish reprend TOUJOURS la plus ancienne entrée `approved` → un compteur `attempts` la passe `failed` après 3 échecs pour ne pas geler la file ; (b) `runEditorialRetry` ARCHIVE (jamais ne ressuscite) les entrées `failed` avec fact-check factuel `blocked:true` (ex. "clapotis") ; (c) **AUCUN timeout client** sur le fetch publish — la publication Meta est NON idempotente et lente, un abort avant `published_at` cause des DOUBLONS ; le guard `agent-drafts.js` `if (published_at) return "Déjà publié"` protège les re-tentatives.
...
- **`kpi-sentinel.js`** (cron `0 9 * * *`) : sentinelle **10 signaux** (occupation 30j, paniers abandonnés, cautions échouées, semaine sans résa via `created_at`, RevPAR -15%, transition saisonnière ≤14j, vs historique `seasonal_memory`, pipeline éditorial <3/14j, **conversion funnel -30%/7j — arch-monitoring 2026-07-04**, **maillon faible du tunnel PAR ÉTAPE + PAR BIEN — signal 10, rituel "1 action/j" 2026-07-18** (`worstStepDrop`/`findWorstBienForStep`), source `conversion_snapshots` snapshot quotidien GA4 via `/api/analytics`, garde anti-bruit `src/utils/conversionDrift.js`) **+ watchdog snapshots manquants** (🔴 si Worker cron mort). Ne pousse que si anomalie ≥🟡 (anti-fatigue). Source occupation = `rm_kpi_snapshots` (OTA inclus), PAS `direct_bookings`.
...
| **Agents IA & mémoire** | `agent_actions`, `agent_drafts`, `agent_memory`, `agent_lessons`, `agent_triggers`, `orchestrations`, `orchestrator_runs`, `action_outcomes`, `backlog_autoverify_log` (I-11, log des fermetures autonomes — distinct de `action_outcomes`), `kpi_history`, `llm_outputs`, `llm_evals`, `ai_ops`, `brain_state` |
| **Contenu / éditorial** | `editorial_calendar` (+ `variation_type`/`concept_id` depuis 2026-07-23, discipline 30/20/50 `ADR-MIX-001`), `editorial_photos`, `property_guides`, `social_bot_log` |
| **Croissance réseaux** (2026-07-23) | `social_snapshots` (1 ligne/plateforme/jour UTC, jamais de faux zéro), `social_growth_reports` (1 rapport/run de l'agent, source de l'onglet Croissance audience) |
...
| **Agent budget pub (brique 3, 2026-07-19)** | `ad_budget_actions` (trace de chaque décision de `ad-budget-execute` : channel/bien/adset, verdict, action pause/increase/decrease, old/new budget cents, mode shadow/live, executed) |
...
| `0 9 * * *` | 9h UTC / 5h MTQ | **`beds24-refresh` (rotation du token Beds24 — rapatriée ici le 2026-07-24, elle n'avait PLUS AUCUN planificateur depuis la mort des jobs cron-job.org du 07-12 ; le token invite-code ne vit que 24h, donc sans elle tout Beds24 tombe, y compris la création de résa Nogent post-Stripe)** · **`morning-brief` (brief matinal ntfy)** · **`kpi-sentinel` (9 signaux + watchdog ntfy)** · **`ad-budget-execute` (agent budget pub, shadow par défaut)** · **`social-insights` (snapshot abonnés 4 plateformes → `social_snapshots`, 2026-07-23)** · ai-ops refresh · monitor · rappels hôte J-7..J+3 · digest arrivées · alertes occupation · `runOccupancySnapshot`→`rm_kpi_snapshots` · gap/yield pricing · `caution-cron` + caution auto-release · inventaire · `devis-solde-cron` · coherence-check · agents-run(all) + orchestrator + eval + digest IA |
...
| `0 6 * * 1` | lundi 6h UTC | rapport hebdo · prix-recap · RAG ingest · agents-execute + digest · token health check · SEO report · bug-triage · **agents-triage** · **`backlog-verify` (2026-07-18, I-11 — ferme les items backlog vérifiables numériquement, silence si 0 fermé)** · memory-distill · guide-write · rm-auto-update?scan=1 · veille-zone-scan · rapport-business · `send-veille-recap` (SÉQUENCÉ après rm-auto-update/veille-zone-scan) · **`rm-price-digest` (2026-07-18, SÉQUENCÉ après rm-auto-update, même contrainte fraîcheur — isole les écarts prix RM ≥12% vs prix live sur 30j, push ntfy, silence si 0 écart)** · **`quality-check-draw` (2026-07-18, tire un bien au hasard pour un contrôle qualité surprise, crée une entrée `maintenance` category='qualite')** · **`social-growth-agent` (2026-07-23, `ADR-SGM-001` — croissance abonnés vs cible, recos organiques advisory, INSÈRE ≤2 posts `planned` dans `editorial_calendar` via le gate, digest ntfy anti-fatigue)** |
...
| ~~—~~ | ~~7h UTC~~ | ~~`/api/beds24-token-watch`~~ → **MIGRÉ dans le Worker `0 9 * * *` le 2026-07-24**, juste APRÈS la rotation (et non plus avant) : si la rotation a réussi il reste ~24h et rien n'est envoyé ; si elle a échoué en silence, cette alerte email est le seul signal. Accepte désormais `POSTSTAY_SECRET`. **Deux bugs corrigés le même jour en le rebranchant** : (1) il lisait `expiryDate`/`expiresAt`/`expire`, alors que Beds24 renvoie `token.expiresIn` en secondes → il sortait toujours sur « expiration absente » et n'a **jamais pu alerter avant expiration** depuis sa création ; (2) seuil passé de **7 jours → 12 heures**, sinon un token de 24h aurait déclenché un mail quotidien sur un token sain (alerte qui crie au loup = alerte ignorée). Vérifié live : `expiresInHours:23`, `alert:false`. |
