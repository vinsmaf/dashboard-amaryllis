...
- **Articles SEO longue traîne** (distinct des guides — D1, pas de fichier JSX par article) : table D1 `seo_articles` (slug/title/meta/content_html/category/status/views) · API CRUD `functions/api/articles.js` · injection meta+JSON-LD `functions/article/[slug].js` · template public `src/ArticlePage.jsx` (route `/article/:slug`) · index `src/ArticlesIndex.jsx` (`/articles`) · édition admin onglet 📝 `src/tabs/ArticlesTab.jsx`. **42 articles publiés** (seed `scripts/seed-articles-30.sql` + 12 antérieurs). Monétisation : `ArticlePage` rend cartes biens RÉSERVER + `EncartActivite` (Viator/GYG, mappé slug/catégorie) + `LienAffilie` DiscoverCars (articles transport/itinéraire) — articles `nogent` exclus. Sitemap : `prerender.mjs` extrait les 30 slugs du seed → `/article/<slug>` (porté de la branche orpheline `claude/sad-bartik-02a3c2`, déployé 2026-06-23, commit `c192ded`).
