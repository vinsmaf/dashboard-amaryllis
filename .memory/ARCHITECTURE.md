...
- **Contacts voyageurs/locataires** (2026-06-24, commit agent `cbb50f6`) : `functions/api/guest-contacts.js` (admin, `verifyBearer`) — GET liste (filtres `statut`/`bien`/`q`/`limit`) / POST création. Table D1 **`guest_contacts`** (migration `migrations/0003_guest_contacts.sql`, ~53 contacts seedés depuis scan WhatsApp). Distinct de `crm_clients` (clients résa) et `contacts` (leads formulaire).
