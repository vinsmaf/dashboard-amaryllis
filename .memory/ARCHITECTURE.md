...
> **Date :** 2026-07-01 · **Statut :** carte de l'état actuel, à maintenir (pas un historique).
...
- **Guides** : `GuidePOI.jsx` (template data-driven unique pour **25 guides POI** via `src/data/guidesPoi.js`) + **32+ guides dédiés routés** (`src/Guide*.jsx` hors hub `Guide.jsx`, template `GuidePOI.jsx`, et `GuideEditor.jsx`) — `GuideDiamant`, `GuideEn` /villa-rental-martinique, `GuideExplorer` /explorer carte, `GuideSejour` /guide-sejour/ livret in-stay, + 11 nouveaux (2026-06-23 : `GuideEnFamille`, `GuideEnCouple`, `GuideSnorkeling`, `GuideSeDeplacer`, `GuideItineraire`, `GuideBudget`, `GuideSainteLucePluie`, `GuideOuLoger`, `GuideNogentQueFaire`, `GuideNogentDormir`, `GuideNogentGuinguettes`) + le hub `Guide.jsx`. **Structure canonique** : `ReadingProgressBar` + `BridgeVilla` + Schema.org `Article` + `MaillageCluster(currentSlug)` (ADR-GUIDES-STD-001). `guidesIndex.js` : prop `dist` contrôle visibilité dans villa fiche.
...
- **CRM réactivation** (2026-06-23, Phase 1 roadmap `docs/crm-roadmap.md`) : `functions/api/crm-lifecycle.js` — `GET ?secret=POSTSTAY_SECRET&segment=winback|fidelite[&dry=1]` (secret **fail-closed**). `winback` = dormants/perdus (dernier_sejour 6-36 mois) ; `fidelite` = accès prioritaire saisonnier (tous anciens). **Iguana exclu** (RM-19). Envoi via **API batch Resend** (anti rate-limit 429). Anti-doublon : table D1 **`crm_campaigns`** (`client_id`+`campaign`). AUCUNE remise codée (RM advisory). **winback envoyé 2026-06-23 (26/26)** ; **fidelite programmé 2026-09-08** (tâche `crm-fidelite-septembre-amaryllis`, mode assisté).
