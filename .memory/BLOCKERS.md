...
## 🤖 2026-06-15 — Bot social « répond aux gens qui cherchent une location » (3 briques livrées, 3 actions Vincent)
- **Construit & déployé** : agent roster `repondeur-social` (ton Amaryllis, garde-fous anti prix/dispo/lien) ; `/api/social-webhook` (auto-post FB/IG, signé, kill-switch `SOCIAL_BOT_DISABLED`, mode `SOCIAL_BOT_MODE` shadow/live) ; `/api/social-poll` (détecteur sans webhook) ; `/api/social-draft` (texte→brouillon) ; `scripts/group-watch.mjs` (veille groupes locale Playwright). Secret `SOCIAL_WEBHOOK_VERIFY_TOKEN` posé. Rédaction testée live (lead villa→réponse on-brand 0.9 ; spam→ignoré). 15 tests. Doc `docs/marketing/social-bot-app-review.md`.
- **MUR Meta découvert** : (a) en mode Développement, Meta **ne livre aucun webhook de prod** → l'auto-post webhook exige **app publiée + App Review** (multi-jours). (b) Le token de page actuel poste mais **n'a pas `pages_read_engagement`** → même la lecture (poller) est bloquée.
- **🔴 Actions Vincent** : (1) **Détecteur AUJOURD'HUI** = régénérer `META_PAGE_TOKEN` avec `pages_read_engagement` (+`instagram_manage_comments`) via Graph API Explorer (Standard Access, page propre = PAS d'App Review), `wrangler pages secret put META_PAGE_TOKEN` → me dire « fait » → je relance `/api/social-poll`. (2) **Auto-post** = soumettre l'**App Review** (dossier prêt). (3) **Veille groupes** = `node scripts/group-watch.mjs --login` + `~/.amaryllis-groups.json` (URLs) + lancer avec `SOCIAL_DRAFT_SECRET=$POSTSTAY_SECRET`.
- **À planifier quand le token marche** : `/api/social-poll?secret=POSTSTAY_SECRET` (cron 15-30 min) + `group-watch.mjs` (launchd local). Caveat veille groupes : scraping DOM fragile + risque ban (lecture seule, session perso) — jamais d'auto-post chez les autres.
