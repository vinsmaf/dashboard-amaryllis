...
- **Dashboard "résa invisible" — RÉSOLU 2026-07-14 (suite)** : cause réelle trouvée — `readAll_()` (GAS) re-téléchargeait les colonnes A/B entières du Sheet à CHAQUE bien (9×, parfois ×4 années via `readHist_`/`revenueSummarySource_`) au lieu d'une fois → 100+ lectures réseau redondantes par `read()`, aggravé par le quota Apps Script sous tension (confirmé par `clasp deploy` lui-même en erreur "Resource has been exhausted"). Fix : `fetchNameCols_()` cache colA/colB une fois par sheet, réutilisé par `findChargesTotalRow_`/`findHistTotalRow_`/`findHistNightsRow_`/`findRowByKeyword_`. Timeout client `syncFromSheets` relevé 20s→50s (mesuré 35,7s réel en charge). Déployé (Apps Script @95 + commit `44479b3`), vérifié : `read()` répond 200 avec toutes les résas.
