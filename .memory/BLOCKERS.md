...
## 🤖 2026-06-15 — Bot social : état après session token (token OK, FB bloqué par Meta policy)
...
- **Token META_PAGE_TOKEN** : ✅ **régénéré le 2026-06-15** avec 20 permissions dont `pages_read_engagement` (permanent, ne périme pas). Graph API Explorer URL params + presse-papier + endpoint `/api/meta-refresh-token` (supprimé post-usage). Vérifié via `debug_token` → scopes OK.
- **IG `/api/social-poll`** ✅ : fonctionne — lit les médias IG, `scanned:0` = pas de commentaires actifs en ce moment.
- **FB `/{pageId}/feed`** 🔴 **bloqué par Meta policy** : requiert `pages_read_engagement` **Advanced Access** (App Review) même avec le token correct. Standard Access ne suffit plus pour le feed endpoint (changement Meta 2023). Le token est bon, le problème est au niveau app.
- **🔴 Action unique restante** : soumettre l'**App Review** (dossier prêt `docs/marketing/social-bot-app-review.md`). 5-10 jours. En attendant → `group-watch.mjs` couvre FB page + groupes via Playwright.
- **Veille groupes** : `~/.amaryllis-groups.json` peuplé (10 groupes). Reste : `node scripts/group-watch.mjs --login` (1 fois) + launchd cron 30min.
