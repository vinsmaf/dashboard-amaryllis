...
- **Conséquence** : les échéances ajoutées à `~/.claude/memory/AGENDA.md` ce jour (26/07, 27/07, 22/08) **ne sont pas poussées vers KV** → le brief matinal ntfy 6h ne les remontera pas tant que ce n'est pas réglé. Le fichier AGENDA.md local, lui, est à jour (le hook de session le lit directement, donc la couche « rappel en début de session » fonctionne).
...
- **Data** : résa Nogent en cours corrigée (Nicolas MBANDJOCK, 288 € → **400 €**) — vérifié dans « Toutes les Réservations » ET « revenus locatif 2026 » (Nogent juillet 5 517,48 → **5 629,48 €**, soit exactement +112 €).
- **À surveiller** : (a) **le rendu d'un Reel converti en Short n'a JAMAIS été jugé par un humain** — le 1ᵉʳ Short partira automatiquement avec le prochain Reel de la rotation (le 25/07 est un carrousel) ; couper avec `YOUTUBE_AUTOPUBLISH=0` si le rendu déçoit. (b) Le token YouTube de Vincent est **insert-only** (délivré avant l'ajout de `youtube.force-ssl`) → l'agent ne peut pas supprimer une vidéo ; recliquer « Connecter YouTube » pour lui donner ce droit. (c) L'impact loop affiche des deltas à 0 tant que l'historique abonnés < 30j (normal, 1 point/jour depuis le 22/07).
