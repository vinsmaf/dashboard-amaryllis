...
- 🟡 **Friction ouverte** : une AUTRE session (spawnée via chip proactive pour le fix `cfRow`) a "traité mais pas commit/déployé" le même sujet, en parallèle — le fix ayant été fait et poussé directement ici, ce travail parallèle est redondant et Vincent l'a abandonné sans committer. Le worktree associé (`.claude/worktrees/charming-neumann-6cb95e/`, si encore présent sur disque) n'a jamais été nettoyé explicitement — pas critique (rien n'a été commité dessus), mais à vérifier/purger à l'occasion si ça traîne.
...
- 🟡 **`patrimoine-dashboard/functions/api/_locatif.js` (ancien pont Sheet) — statut de bascule non confirmé** : `ARCHITECTURE.md` documente désormais `/api/revenue-summary` comme LE pont cross-projet, mais je n'ai aucune visibilité sur si le code patrimoine a réellement basculé dessus ou s'il garde les deux chemins actifs en parallèle (l'ancien parseur Sheet + le nouvel endpoint). À confirmer côté instance patrimoine.
