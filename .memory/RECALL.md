...
| **Réservations / annulations / revenus** | Lire **`.memory/FLUX-RESAS.md`** (pipeline validé 27/06 — 4 canaux, anti-patterns, commandes curl). En résumé : Airbnb/Booking = Worker iCal hourly auto · Direct = stripe-webhook + annulation MANUELLE · Beds24 = webhook Nogent. **Annulation manuelle via Claude** = 2 appels séparés : `deleteReservation` puis `revenus2026RebuildBienApply` (cancelReservations_ en 1 appel = timeout proxy). JAMAIS `revenus2026FromMonth(ignoreMemo:true)` = BLOQUÉ. | FLUX-RESAS.md · LEARNINGS |
