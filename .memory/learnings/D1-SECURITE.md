...
## 🔴 Un secret réel collé en clair dans un fichier `.memory/` versionné reste EN GIT même après édition — 2026-07-16
- **Piège trouvé** : `.memory/FLUX-RESAS.md` contenait `SECRET="0e091781cd00c38efa118d36f46e6bccbb0d713a6918e23e"` (CLAUDE_SECRET, Bearer admin) en clair dans un bloc `bash` d'exemple, collé lors d'une session antérieure (5 commits touchant ce fichier depuis). `.memory/` n'est PAS gitignoré dans ce projet (contrairement à `.dev.vars`) — le secret est donc dans l'historique git depuis plusieurs commits, pas juste "dans un fichier local".
- **La prochaine fois** : ne JAMAIS coller la valeur réelle d'un secret dans un fichier `.memory/`/doc versionné, même pour un exemple de commande — toujours `SECRET=$(grep -m1 '^X_SECRET' .dev.vars | cut -d= -f2- | tr -d '"')` dans les exemples de code, jamais la valeur en dur. Retirer le secret d'un fichier ACTUEL ne le retire pas de l'historique git — si un secret y est trouvé, le signaler explicitement à l'utilisateur pour rotation, ne pas se contenter de le redacter en silence.
...
## Un partenaire Meta Business compromis peut saboter les RÉGLAGES (pas seulement dépenser de l'argent) — 2026-07-21
- **Piège** : après le hack Meta de juin (partenaire frauduleux "Businesss Meta", moyens de paiement supprimés en urgence, partenaire retiré le 07-19), le compte semblait assaini. Un **2ᵉ partenaire compromis**, présent dès la création de la campagne "Amaryllis campagnes", est resté invisible car il n'a **rien dépensé** — il a juste modifié la restriction géographique de la Page ("Show only to certain countries: Vietnam"), rendant la Page invisible en France/Martinique et bloquant silencieusement la diffusion des annonces (`effective_status: WITH_ISSUES`, spend quasi nul malgré budget actif). Repéré uniquement parce que le budget pub agent (`ad_budget_actions` shadow log) montrait 8 évaluations consécutives sans aucun signal (`verdict:idle` partout) — symptôme d'absence totale de données, pas d'un vrai "pas assez de volume".
- **La prochaine fois** : après un incident de sécurité de compte pub, **ne pas se limiter à vérifier les moyens de paiement et le 1er partenaire trouvé** — auditer TOUS les partenaires + Utilisateurs Système dans Meta Business Settings, et vérifier aussi les réglages non-financiers à risque (restriction géo/audience de la Page, permissions Page, domaines vérifiés). Un compte "débloqué" au sens paiement peut rester saboté au sens ciblage.
- **Diagnostic utile** : si un agent shadow/monitoring tourne plusieurs jours sans jamais produire de signal alors qu'un budget est censé être actif, vérifier `effective_status` réel des ads (pas juste campagne/adset) avant de conclure "pas assez de volume" — `WITH_ISSUES` explique un silence total, une insuffisance de volume donnerait normalement au moins quelques impressions/clics cohérents avec le budget.
