...
## `context.waitUntil()` ANNULE (sans erreur) toute tâche de fond qui dépasse son budget → un `.catch()` de log ne se déclenche JAMAIS — 2026-07-17
- **Piège** : un cache `sheets-proxy` est resté figé 17,5h. Cause : `context.waitUntil(refreshReadCache(...))` où le refresh dure ~32s. Cloudflare a répondu (via `wrangler pages deployment tail`) : « waitUntil() tasks did not complete within the allowed time after invocation end and have been cancelled ». **0 succès sur 6 tentatives observées.** Piège DANS le piège : la tâche est **annulée**, pas en erreur → **aucune exception levée** → le `.catch(() => log(...))` ajouté pour surveiller l'échec ne s'est jamais déclenché. C'était une illusion de sécurité — on croyait surveiller un mécanisme qui n'aboutissait jamais, en regardant un log vide.
- **Fix / la prochaine fois** : (1) ne PAS mettre en `waitUntil` une tâche dont la durée peut dépasser quelques secondes (un fetch tiers lent, un calcul lourd) — elle sera silencieusement tuée. Pour du travail long post-réponse, c'est un cron Worker qu'il faut, pas `waitUntil`. (2) Une absence de log n'est PAS une preuve de succès quand le mécanisme peut être annulé sans exception — c'est souvent le signal inverse. Vérifier le RÉSULTAT (ici : l'âge du cache n'était jamais retombé), pas la présence/absence d'erreur. (3) `wrangler pages deployment tail <deployment-id>` capture ces messages de la runtime elle-même — l'outil de diagnostic à sortir quand un comportement de fond est suspect.
- **Nuance** : le stale-while-revalidate en `waitUntil` reste valable quand le refresh est COURT (ex. `revenue-summary.js`, calcul bien plus rapide que les ~32s de `sheets-proxy read`). Le pattern n'est pas mauvais en soi — il est mauvais quand la tâche dépasse le budget. Ne pas le retirer partout par analogie.

## `.dev.vars` local peut diverger silencieusement des vrais secrets Cloudflare Pages — ne jamais s'y fier pour un diagnostic de prod — 2026-07-15
- **Piège** : en diagnostiquant "le rapprochement Stripe ne marche plus", un test direct `curl api.stripe.com` avec `STRIPE_SECRET_KEY` lu depuis `.dev.vars` local a renvoyé UN SEUL payout (daté de 3 semaines plus tôt) — complètement différent de ce que l'endpoint de prod (`/api/stripe-reconcile`, qui utilise `env.STRIPE_SECRET_KEY` = le vrai secret Cloudflare Pages) retournait. `.dev.vars` n'est pas synchronisé automatiquement avec les secrets configurés dans le dashboard Cloudflare — il peut contenir une clé périmée/différente sans qu'aucun signal ne le dise.
- **Fix / la prochaine fois** : pour tout diagnostic touchant un secret potentiellement sensible à la fraîcheur (clé API tierce, token), passer par l'endpoint de PROD lui-même (curl direct, ou ajouter un paramètre `?debug=`/`?balance=` temporaire à l'endpoint concerné) plutôt que de reproduire l'appel en local avec `.dev.vars`. Un résultat local qui contredit le comportement observé en prod est un signal que la clé locale diverge, pas nécessairement que le comportement prod est buggé.

## Sentry (`main.jsx`) est un pipeline de monitoring TOTALEMENT SÉPARÉ de `bugCapture.js`/`client_errors` — un fix sur l'un ne couvre jamais l'autre — 2026-07-15
- **Piège** : 2 alertes Sentry "high priority" reçues par email pour un faux positif de déploiement (chunk JS périmé) déjà supposé réglé — en fait le fix du matin même (`isStaleChunkError`, commit `d4c3984`) n'avait été appliqué qu'à `bugCapture.js` (pipeline maison → D1 `client_errors` → onglet 🐞 Bugs), jamais à Sentry, qui a sa PROPRE liste `ignoreErrors` (`main.jsx`) totalement indépendante et qui ne couvrait pas ce pattern précis (message générique `.default`, seulement les patterns explicites type `ChunkLoadError`).
- **Fix / la prochaine fois** : quand on corrige un pattern de bruit/faux positif, vérifier s'il existe PLUSIEURS pipelines de monitoring en parallèle (`grep -ri sentry` avant de conclure qu'un seul système existe) — ici découvert que `@sentry/react` est bien installé et initialisé, sans que ce soit documenté ailleurs avant cette session. Réutiliser la même fonction de détection (`beforeSend` → `isStaleChunkError`) plutôt que dupliquer une regex, pour ne pas re-diverger la prochaine fois.
...
## CI GitHub Actions a un filtre `paths:` — un push qui ne le touche pas ne déclenche RIEN (2026-07-10)
- **`.github/workflows/deploy.yml` a un `paths:` scopé à `src/**`, `functions/**`, `public/**`, `workers/**`, `scripts/**`, `index.html`, `package*.json`.** Un commit qui touche UNIQUEMENT `appscript/`, `.memory/`, `CLAUDE.md` ne déclenche AUCUN run — comportement normal, pas une CI cassée. Vécu : un `Monitor` posé pour suivre le déploiement d'un commit doc-only a tourné à vide jusqu'au timeout (5min), avant de comprendre que rien n'allait jamais se déclencher. **La prochaine fois : avant de poser un Monitor sur "le déploiement d'un commit", vérifier `git diff --stat` du commit contre le `paths:` du workflow** — si aucun fichier ne matche, ne pas attendre de run, le fix (GAS via clasp, doc pure) est déjà appliqué par un autre mécanisme ou n'a simplement rien à déployer côté Pages.
