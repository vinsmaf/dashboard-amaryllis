...
## 🔴 3 notions de « prix » à ne jamais confondre — confondues plusieurs fois dans la même session — 2026-07-10
...
- **Découvert en construisant `parity-check.js`** (contrôle de parité tarifaire direct vs Booking) : j'ai proposé successivement le prix de base puis la reco RM avant que Vincent tranche explicitement pour le prix réel de l'onglet Tarifs — la confusion s'était déjà produite plusieurs fois avant dans la même session sur ce même sujet.
- **La prochaine fois** : toute question « quel est le prix du bien X à la date Y » = lire `/api/site-config?type=prices` avec fallback `biens.js` si absent (jamais `rm-recommendations` seul, jamais `biens.js` seul). Détail complet : `CLAUDE.md` section « 1bis. Pricing — 3 notions à ne JAMAIS confondre ».
