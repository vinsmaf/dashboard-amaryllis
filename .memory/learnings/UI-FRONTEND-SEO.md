...
## Un sitemap n'est relu par Google que sur déclencheur — pas en continu — resoumettre force une redécouverte immédiate — 2026-07-15
- **Piège vécu** : 37 articles publiés le 23/06, sitemap soumis le même jour, toujours 0 impression/0 clic sur les 3 mois de rapport GSC 3 semaines après. Diagnostic initial supposé (contenu/qualité/maillage interne) était FAUX — l'Inspection d'URL GSC sur 2 articles-échantillon a montré "Google ne reconnaît pas cette URL" + "aucun sitemap référent détecté". Cause réelle : Google avait lu le sitemap **une seule fois**, le jour du déploiement, et jamais depuis — les nouvelles URLs qui y étaient déjà présentes ce jour-là n'ont, pour une raison non élucidée, pas toutes été prises en compte à cette lecture unique.
- **Fix / la prochaine fois** : avant de conclure à un problème de contenu/classement, vérifier l'Inspection d'URL GSC sur 1-2 pages représentatives — "Google ne reconnaît pas cette URL" + "aucun sitemap référent" est un signal net et sans ambiguïté de non-découverte, très différent de "indexée mais mal classée". Le fix est trivial et gratuit : resoumettre l'URL du sitemap dans Search Console → Sitemaps force une relecture immédiate (vérifié : nombre de "pages découvertes" passé de 64 à 99 en quelques secondes). Aucun automatisme actuel ne déclenche cette relecture — à surveiller si un futur lot de contenu subit le même sort.
- **Piège UI Search Console avec l'automation navigateur** : le menu "Ajouter un filtre" + la pagination "Lignes par page" ont mal réagi aux clics par coordonnées (menu qui se referme sans effet, valeur qui ne change pas) — passer par l'onglet natif "PAGES"/pagination `>` (boutons stables) plutôt que les dropdowns custom a été plus fiable. `get_page_text` peut aussi rester bloqué sur un ancien rendu DOM résiduel après un changement de vue SPA (Search Console) — se fier au screenshot dans ce cas, pas au texte extrait.

## 🌓 Token CSS qui s'inverse avec le thème + fond FIXE = texte invisible garanti dans un des deux modes — 2026-07-12
- **Piège vécu** : `--c-ivory` s'inverse avec le thème clair/sombre (clair `#faf5e9` en light, quasi-noir `#111c1b` en dark — conçu comme alias de "fond de page", pas comme une couleur littérale). Le footer Contact a un fond CSS **figé** (`#072626`, ne suit PAS le thème) mais utilisait `var(--c-ivory)` pour son texte → invisible dès que le site passe en dark mode (texte quasi-noir sur fond déjà sombre). Signalé par Vincent comme "le bouton contact ne marche pas" — en réalité le clic marchait, c'est la section d'arrivée qui était illisible.
- **La prochaine fois** : avant d'utiliser une couleur de texte sur une section à fond FIXE (hardcodé, indépendant du thème), vérifier si le token choisi s'inverse aussi avec le thème — si oui, utiliser un token explicitement "theme-invariant" (`--fg-on-ink` dans ce projet : résout à une couleur claire dans les DEUX thèmes). `--c-ivory`/`--c-navy` sont conçus pour s'inverser ENSEMBLE avec un fond qui suit le même thème — les utiliser sur un fond qui ne suit pas le thème casse la paire.
- **Diagnostic** : un dark mode forcé (`document.documentElement.setAttribute("data-theme","dark")` en JS) + lecture de `getComputedStyle(el).color` révèle immédiatement la vraie couleur résolue — plus fiable qu'un screenshot si l'outil de capture a des artefacts à certaines profondeurs de scroll (vu ce jour : screenshot noir uniforme en viewport desktop à scroll profond, correct en mobile — probablement un bug du tool de preview, pas du site, confirmé par lecture DOM directe).

## 🖱️ Popup séparé de son déclencheur par un gap = besoin d'un hover-intent, jamais un onMouseLeave instantané — 2026-07-12
- **Piège vécu** : `HoverContact` ouvre un popup (WhatsApp/Email) au survol, mais le popup est positionné à `calc(100% + 10px)` de son déclencheur — un gap de 10px que la souris doit traverser pour l'atteindre. `onMouseLeave` synchrone (`setShow(false)`) fermait le popup dès que le curseur quittait le déclencheur, AVANT d'atteindre le popup lui-même. Signalé par Vincent : "ça disparaît avant que je puisse cliquer".
- **La prochaine fois** : tout popup/menu séparé visuellement de son déclencheur (par un gap, même petit) doit fermer avec un délai annulable (`setTimeout` + `clearTimeout` au re-entrée), jamais un `onMouseLeave` synchrone. Pattern minimal : `openNow()` annule le timer pendant et rouvre ; `closeSoon()` programme la fermeture (~300ms suffit pour un déplacement de souris humain sur quelques px) ; le timer se réinitialise si la souris rentre dans le déclencheur OU le popup (les deux sont dans le même wrapper avec les handlers).
- **Vérification fiable** : simuler `mouseover`/`mouseout` directement en JS avec des `await new Promise(r => setTimeout(r, Nms))` entre chaque étape donne un résultat déterministe et rapide — beaucoup plus fiable que des clics `computer`/`hover` séquentiels dont la latence réseau entre appels peut dépasser le délai testé, produisant des faux négatifs/positifs incohérents d'un essai à l'autre.
...
## Un badge/résumé UI recalculé indépendamment de la logique de résa réelle peut mentir sans jamais planter — 2026-07-18
- **Mabouya n'avait vendu aucune nuit sur 3 mois (mai-juillet 2026)** malgré un trafic réel (GA4 : 66 sessions/30j) et une durée de session anormalement courte (84s vs 130-210s sur les autres biens — signal de rebond rapide, pas d'absence d'intérêt). Cause trouvée en ouvrant la page en live (pas en lisant le code seul) : le bandeau "Libre dès le X" (`useAvailBadge`) cherchait un créneau de **7 nuits consécutives codé en dur**, sans lien avec `getMinNights(bienId, date)` — la fonction qui calcule le VRAI minimum de séjour (2-7 nuits selon la saison pour Mabouya). Résultat : le bandeau affichait "pas disponible avant le 4 août" alors que le calendrier juste en dessous, sur la MÊME page, proposait activement une réservation valide dès le 19 juillet. Deux widgets, deux sources de vérité différentes, un seul visiteur qui voit une contradiction et part.
- **Pourquoi ça ne plante jamais** : ce n'est pas un bug qui lève une exception ou un test qui casse — c'est un CALCUL PARALLÈLE, syntaxiquement valide, qui donne juste une réponse fausse. Aucun test, aucun `client_errors`, aucun code-review LLM ne l'aurait vu — seule l'ouverture de la page en live avec un œil qui compare les 2 signaux visibles l'a révélé.
- **Leçon généralisable** : dès qu'un bandeau/résumé/badge affiche une conclusion dérivée d'une règle métier (ici : "disponible = respecte le séjour minimum"), vérifier qu'il appelle la MÊME fonction que le composant qui applique réellement cette règle (ici `nextAvailWindow`, déjà utilisée ailleurs pour calculer les vraies dates réservables) — ne jamais laisser un 2ᵉ calcul dupliqué avec un seuil "approximatif" codé en dur. Un chiffre affiché qui ne sert "qu'à indiquer une tendance" finit toujours par être pris au pied de la lettre par un visiteur, et par contredire silencieusement l'élément fonctionnel juste à côté.
- **Symptôme à surveiller pour repérer ce genre de bug ailleurs** : une durée de session anormalement COURTE sur une page par ailleurs comparable (même structure, même trafic) à d'autres pages qui convertissent — c'est un signal de rebond rapide dû à un message décourageant en haut de page, pas juste "moins d'intérêt".
