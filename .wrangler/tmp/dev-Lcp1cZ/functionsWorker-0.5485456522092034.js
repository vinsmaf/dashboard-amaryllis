...
// .wrangler/tmp/pages-cH1Mnf/functionsWorker-0.5485456522092034.mjs
...
var json = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS }), "json");
...
async function onRequest(context3) {
  const { request, env: env22 } = context3;
  ...
  if (request.method === "GET") {
    ...
    let guide = null;
    let source = "static";
    const db = env22.revenue_manager;
    if (db) {
      try {
  ...
  if (request.method === "POST") {
    ...
    const { property_id: pid22, guide } = body;
    if (!pid22 || !guide) return json({ error: "property_id and guide required" }, 400);
    await ensureTable(db);
    const now = Date.now();
    ...
    `).bind(pid22, content_json, now).run();
    return json({ ok: true, property_id: pid22, saved_at: now });
  }
...
var json2 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS2 }), "json");
...
__name2(handleSnapshot, "handleSnapshot");
...
__name2(handleRecalculateSignals, "handleRecalculateSignals");
...
__name2(handleGetSignals, "handleGetSignals");
function parseCSV(text) {
  ...
  for (const raw of text.split("\n")) {
    ...
    const name = cols[1] || "";
    const platform22 = cols[2] || "airbnb";
...
__name2(handleImportListings, "handleImportListings");
async function handleExportListings(db, url) {
  ...
  const getPriority = /* @__PURE__ */ __name2((score) => {
...
__name2(handleExportListings, "handleExportListings");
...
var json3 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS3 }), "json");
...
__name2(findSeasonalProfile, "findSeasonalProfile");
...
__name2(getBasePrice, "getBasePrice");
...
function calcDateReco({
  ...
  const profile32 = findSeasonalProfile(profiles, dateStr);
  const seasonType = profile32 ? profile32.season_type : "mid";
  const profileFound = !!profile32;
  let basePrice = profile32 ? profile32.base_price_override || getBasePrice(property, seasonType) : getBasePrice(property, "mid");
  let minStay = profile32 ? profile32.min_stay_override || getMinStay(property, seasonType) : property.min_stay_default;
  let adjWeekend = 0;
  let adjHoliday = 0;
  let adjEvent = 0;
  let adjLeadTime = 0;
...
__name2(calcDateReco, "calcDateReco");
...
__name2(handleCalculate, "handleCalculate");
...
__name2(handleDeleteOverride, "handleDeleteOverride");
...
var json4 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS4 }), "json");
...
var json5 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS5 }), "json");
...
__name2(triggerApifyRun, "triggerApifyRun");
...
__name2(ensureScrapingConfigs, "ensureScrapingConfigs");
async function handlePost2(db, env22, body) {
  const { property_id, listing_ids } = body;
  if (!property_id) return json5({ error: "property_id required" }, 400);
  const apifyToken = env22.APIFY_TOKEN;
...
__name2(handleGetStatus, "handleGetStatus");
async function onRequest5(context3) {
  ...
  try {
    ...
    if (request.method === "POST") {
      const body = await request.json().catch(() => ({}));
      return handlePost2(db, env22, body);
    }
...
async function rateLimit(db, { key, limit = 5, windowSec = 900 }) {
  ...
  try {
    ...
    return { ok: true, remaining: limit - count32 - 1, retryAfter: 0 };
  } catch (err2) {
...
var json6 = /* @__PURE__ */ __name2((d, s = 200, req) => new Response(JSON.stringify(d), { status: s, headers: corsHeaders(req?.headers?.get("Origin") || "") }), "json");
...
async function onRequestPost(context3) {
  ...
  const rl = await rateLimit(env22.revenue_manager, {
    key: `admin-auth:${ip}`,
    limit: 5,
    windowSec: 900
  });
  ...
  const adminPwd = env22.ADMIN_PWD;
  const menagePwd = env22.ADMIN_PWD_MENAGE;
  if (!adminPwd) {
...
async function onRequestPost2(context3) {
  const { request, env: env22 } = context3;
  const apiKey = env22.ANTHROPIC_API_KEY;
  if (!apiKey) {
...
__name2(getListingPrices, "getListingPrices");
...
async function onRequestGet2(context3) {
  const { env: env22 } = context3;
  const propertyId = env22.GA4_PROPERTY_ID;
  const clientEmail = env22.GA4_CLIENT_EMAIL;
  const privateKey = env22.GA4_PRIVATE_KEY;
...
__name2(parseReport, "parseReport");
...
__name2(ensureTokenTable, "ensureTokenTable");
async function onRequestGet3(context3) {
  const { env: env22, request } = context3;
  const refreshSecret = env22.BEDS24_REFRESH_SECRET;
  if (refreshSecret) {
    const url = new URL(request.url);
  ...
  const currentToken = await getActiveBeds24Token(env22, db);
  ...
  try {
    ...
    if (env22.NTFY_TOPIC) {
      await fetch(`https://ntfy.sh/${env22.NTFY_TOPIC}`, {
        method: "POST",
...
async function getActiveBeds24Token(env22, db) {
  if (db) {
    try {
  ...
  return env22.BEDS24_TOKEN || null;
}
__name(getActiveBeds24Token, "getActiveBeds24Token");
__name2(getActiveBeds24Token, "getActiveBeds24Token");
...
async function onRequestGet4(context3) {
  const { request, env: env22 } = context3;
  const token = await getActiveBeds24Token(env22, env22.revenue_manager);
  if (!token) {
  ...
  const pick = /* @__PURE__ */ __name2((v2key, paramKey) => {
  ...
  const normalize = /* @__PURE__ */ __name2((b) => ({
    bookingId: b.id,
...
__name2(lastNightFrom, "lastNightFrom");
...
__name2(nightsCount, "nightsCount");
...
__name2(statusCodeFrom, "statusCodeFrom");
...
__name2(statusLabel, "statusLabel");
...
__name2(channelLabel, "channelLabel");
...
var json11 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS6 }), "json");
...
async function onRequestPost3(context3) {
  const { request, env: env22 } = context3;
  let token;
  if (env22.BEDS24_REFRESH_TOKEN) {
    try {
      token = await getAccessToken2(env22.BEDS24_REFRESH_TOKEN);
    } catch (e) {
      return json11({ error: `Auth Beds24 \xE9chou\xE9e: ${e.message}` }, 500);
    }
  } else if (env22.BEDS24_TOKEN) {
    token = env22.BEDS24_TOKEN;
  } else {
...
var json12 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS7 }), "json");
...
async function onRequestPost4(context3) {
  const { request, env: env22 } = context3;
  const token = env22.BEDS24_TOKEN;
...
var json13 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS8 }), "json");
...
async function onRequestPost5(context3) {
  const { request, env: env22 } = context3;
  const apiKey = env22.BEDS24_API_KEY;
  const propKey = env22.BEDS24_PROP_KEY;
  const scriptUrl = env22.APPS_SCRIPT_URL;
  const webhookSecret = env22.BEDS24_WEBHOOK_SECRET;
...
__name2(normalizeBooking, "normalizeBooking");
...
__name(statusLabel2, "statusLabel2");
__name2(statusLabel2, "statusLabel");
...
__name(channelLabel2, "channelLabel2");
__name2(channelLabel2, "channelLabel");
...
var json15 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS10 }), "json");
var NOMS = {
...
async function onRequestPost6(context3) {
  const { request, env: env22 } = context3;
  const sk = env22.STRIPE_SECRET_KEY;
...
__name2(ensureContactsTable, "ensureContactsTable");
async function sendNtfyLead(env22, { nom, email, bien, reason = "Resend KO" }) {
  const topic = env22.NTFY_TOPIC;
  if (!topic) return;
...
__name2(sendNtfyLead, "sendNtfyLead");
async function onRequestPost8(context3) {
  try {
    ...
    const esc = /* @__PURE__ */ __name2((s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;"), "esc");
...
var json16 = /* @__PURE__ */ __name2((d, s = 200) => Response.json(d, {
  status: s,
  headers: { "Content-Type": "application/json" }
}), "json");
...
async function onRequestGet9(context3) {
  const { env: env22, request } = context3;
  const secret = env22.CONTACTS_ALERT_SECRET;
  if (secret) {
    const url = new URL(request.url);
  ...
  const ntfyTopic = env22.NTFY_TOPIC;
  try {
...
async function onRequestGet12(context3) {
  ...
  try {
    if (!env22.RESEND_API_KEY) throw new Error("RESEND_API_KEY manquant");
...
var json21 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS12 }), "json");
var BIEN_LABELS = {
  amaryllis: "Villa Amaryllis",
  schoelcher: "Bellevue Sch\u0153lcher",
  geko: "G\xE9ko",
...
async function onRequestPost12(context3) {
  ...
  const emailSent = await sendEmail(env22, bienNom, dateList, minPrice, year);
  const ntfySent = await sendNtfy(env22, bienNom, dateList, minPrice);
...
async function sendEmail(env22, bienNom, dates, minPrice, year) {
  if (!env22.RESEND_API_KEY) return false;
  const dest = env22.NOTIFICATION_EMAIL || "contact@villamaryllis.com";
...
async function sendNtfy(env22, bienNom, dates, minPrice) {
  const topic = env22.NTFY_TOPIC;
...
async function onRequestPost13(context3) {
  ...
  let scriptUrl = env22.APPS_SCRIPT_URL;
...
__name2(forwardChunked, "forwardChunked");
...
async function getBeds24Token(env22) {
  if (env22.BEDS24_REFRESH_TOKEN) {
    const res = await fetch(BEDS24_AUTH_TOKEN2, { headers: { refreshToken: env22.BEDS24_REFRESH_TOKEN } });
    const data = await res.json();
    if (!data.token) throw new Error("Beds24 refresh token invalide ou expir\xE9");
    return data.token;
  }
  if (env22.BEDS24_TOKEN) return env22.BEDS24_TOKEN;
  throw new Error("BEDS24_TOKEN ou BEDS24_REFRESH_TOKEN manquant");
}
__name(getBeds24Token, "getBeds24Token");
__name2(getBeds24Token, "getBeds24Token");
async function confirmBeds24Booking(bookingId, env22) {
  const token = await getBeds24Token(env22);
...
__name2(confirmBeds24Booking, "confirmBeds24Booking");
var json23 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), {
  status: s,
  headers: { "Content-Type": "application/json" }
}), "json");
var NOMS2 = {
...
async function sendEmail2(env22, { subject, html, to }) {
  if (!env22.RESEND_API_KEY) {
    console.error("[webhook] RESEND_API_KEY absent \u2014 email non envoy\xE9");
    return;
  }
  const recipient = to || env22.NOTIFICATION_EMAIL || "contact@villamaryllis.com";
  const r = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${env22.RESEND_API_KEY}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      from: env22.RESEND_FROM || "Amaryllis <notifications@mail.villamaryllis.com>",
...
async function sendConfirmationToGuest(env22, { bienNom, voyageur, email, checkin, checkout, amount, bookingId }) {
  if (!email || !email.includes("@")) {
    console.log("[webhook] Pas d'email voyageur \u2014 confirmation non envoy\xE9e");
    return;
  }
  await sendEmail2(env22, {
    to: email,
...
__name2(sendConfirmationToGuest, "sendConfirmationToGuest");
async function onRequestPost14(context3) {
  const { request, env: env22 } = context3;
  const rawBody = await request.text();
  const sig = request.headers.get("stripe-signature");
  if (!env22.STRIPE_WEBHOOK_SECRET) {
  ...
  const valid = await verifyStripeSignature(rawBody, sig, env22.STRIPE_WEBHOOK_SECRET);
  if (!valid) {
  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    if (bookingId) {
      try {
        await confirmBeds24Booking(bookingId, env22);
    ...
    await sendConfirmationToGuest(env22, { bienNom, voyageur, email: guestEmail, checkin, checkout, amount, bookingId });
...
async function onRequestGet14(context3) {
  const { request, env: env22 } = context3;
  const apiKey = env22.OPENWEATHER_API_KEY;
...
var json25 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS13 }), "json");
var DDL = `
...
var json26 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS14 }), "json");
var AGENTS = [
  {
...
__name2(fetchAgentHistory, "fetchAgentHistory");
function buildPrompt(agent, history = []) {
  ...
  const fmt = /* @__PURE__ */ __name2((items) => items.map(
...
async function onRequest10(context3) {
  ...
  const secret = env22.PURGE_SECRET;
  ...
  try {
    ...
    const count32 = toDelete?.length ?? 0;
    if (count32 > 0) {
      await db.prepare(
    ...
    console.log(`[contacts-purge] ${count32} contact(s) supprim\xE9(s) ant\xE9rieurs au ${cutoffIso}`);
    return json27({
      ok: true,
      deleted: count32,
      cutoff_iso: cutoffIso,
      message: count32 > 0 ? `${count32} contact(s) ant\xE9rieur(s) au ${cutoffIso} supprim\xE9(s) (RGPD \u2014 2 ans max)` : `Aucun contact \xE0 purger (seuil : ${cutoffIso})`
    });
  } catch (err2) {
...
async function onRequest11(context3) {
  ...
  const jsonErr = /* @__PURE__ */ __name2((msg, status = 400) => new Response(JSON.stringify({ error: msg }), {
    status,
    headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
  }), "jsonErr");
...
function icalBienMap(env22) {
  return {
    airbnb: {
      amaryllis: env22.ICAL_AMARYLLIS,
      schoelcher: env22.ICAL_SCHOELCHER,
      geko: env22.ICAL_GEKO,
      mabouya: env22.ICAL_MABOUYA,
      iguana: env22.ICAL_IGUANA,
      zandoli: env22.ICAL_ZANDOLI,
      nogent: env22.ICAL_NOGENT
    },
    booking: {
      amaryllis: env22.ICAL_BOOKING_AMARYLLIS,
      schoelcher: env22.ICAL_BOOKING_SCHOELCHER,
      geko: env22.ICAL_BOOKING_GEKO,
      mabouya: env22.ICAL_BOOKING_MABOUYA,
      iguana: env22.ICAL_BOOKING_IGUANA,
      zandoli: env22.ICAL_BOOKING_ZANDOLI,
      nogent: env22.ICAL_BOOKING_NOGENT
    }
  };
}
__name(icalBienMap, "icalBienMap");
__name2(icalBienMap, "icalBienMap");
...
function parseIcal(text) {
  ...
  for (const ev of text.split("BEGIN:VEVENT").slice(1)) {
    ...
    const fmt = /* @__PURE__ */ __name2((s) => `${s.slice(0, 4)}-${s.slice(4, 6)}-${s.slice(6, 8)}`, "fmt");
...
__name2(json28, "json");
async function fetchBeds24Blocked(env22) {
  const token = env22.BEDS24_TOKEN;
...
__name2(fetchBeds24Blocked, "fetchBeds24Blocked");
async function onRequest12(context3) {
  ...
  if (bienId === "nogent") {
    const cached = env22.AVAIL_CACHE ? await env22.AVAIL_CACHE.get(cacheKey, "json") : null;
    if (cached) return json28({ ...cached, fromCache: true });
    const blocked = await fetchBeds24Blocked(env22);
    const result2 = {
      blockedDates: Array.from(blocked).sort(),
      sources: { beds24: { ok: blocked.size >= 0, count: blocked.size } }
    };
    if (env22.AVAIL_CACHE) {
      await env22.AVAIL_CACHE.put(cacheKey, JSON.stringify(result2), { expirationTtl: 600 });
    }
  ...
  if (env22.AVAIL_CACHE && !bookingUrlSafe) {
    await env22.AVAIL_CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: 600 });
  }
...
var json31 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS15 }), "json");
...
var json32 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS16 }), "json");
var DDL2 = `
...
var json33 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS17 }), "json");
...
var json34 = /* @__PURE__ */ __name2((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: CORS18 }), "json");
...
function buildMeta(title22, desc, url, image) {
  const t = escHtml(title22);
...
async function onRequest20(context3) {
  ...
  if (BIENS[slug]) {
    ...
    const title22 = `${bien.nom} \u2014 Location ${bien.lieu} \xE0 partir de ${bien.prix}\u20AC/nuit`;
    ...
    const meta = buildMeta(title22, desc, url, image);
...
// .wrangler/tmp/bundle-7dU8KX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7dU8KX/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5485456522092034.js.map
