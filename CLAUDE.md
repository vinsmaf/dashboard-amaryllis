...
### 1ter. `BIENS_MAP.cfRow` (Sheet "revenus locatif 2026") pointe sur la ligne CHARGES, pas cashflow (trouvé 07/2026, jamais corrigé)

`appscript/SCRIPT_SHEETS.js` → `BIENS_MAP[i].cfRow` est censé désigner la ligne "cashflow" de chaque bien, consommée par `readAll_()` → `biens[i].cashflow` → **affichée en prod dans `Cockpit.jsx`** ("CF YTD" KPI top-level, alerte cashflow négatif Nogent, `statutBien()` pour le badge vert/jaune/rouge). Vérifié en live le 2026-07-10 (chantier `/api/revenue-summary` charges/cashflow) : `cfRow` pointe en fait sur la ligne **"total X"** du bloc dépenses (le total des CHARGES, pas `ca-charges`), et `getVals(sheet, b.cfRow, 2, 12)` lit en plus à partir de la colonne **B** (le libellé texte, ex. "total nogent") au lieu de C (janvier) — décalage d'un mois **et** décembre jamais lu. Conséquence concrète observée : le tableau `cashflow` actuel est une série de charges à peu près stables et **toujours positives** (jamais négatif même hors-saison), alors que le vrai cashflow (`ca - charges`, recalculé correctement pour `/api/revenue-summary` via recherche dynamique — `findChargesTotalRow_`/`readChargesBlock_`) oscille fortement avec l'occupation, y compris négatif en basse saison. Ça biaise `statutBien()` vers "vert" (le seuil `cf > 0` est presque toujours vrai) et rend l'alerte "cashflow négatif consécutif" de Nogent quasi inerte. **Non corrigé dans ce chantier** (périmètre différent — `readAll_()`/`Cockpit.jsx`/`calculations.js`, pas `revenue-summary.js`) : signalé à Vincent, correctif à faire séparément si validé. Ligne réelle cashflow = `cfRow + 1` (vérifié 7 biens + T4, 2025 et 2026).
