...
| `/api/ad-budget-agent` | GET | `ad-budget-agent.js` | **Agent budget pub, brique 2 (DÉCISION, ADVISORY STRICT)** — croise le modèle CAC par bien (`src/utils/adBudgetAgent.js`, CAC plafond = 50 % de la commission Booking nette évitée, RM-08) avec les perfs Meta (`meta-ads-insights` niveau ad set) → **vision budget par bien** (`?budget=600` : combien dépenser par bien, seuls Amaryllis/Zandoli/Géko éligibles) + arbitrage scale/hold/cut. **Ne modifie AUCUN budget, ne dépense RIEN** (l'exécution = brique 3, à construire). Auth Bearer admin. |
