...
| `/api/create-payment-link` | POST | `create-payment-link.js` | Crée un Stripe Payment Link pour devis WhatsApp (montant en centimes, bien, dates, type acompte/solde/total). Auth Bearer admin. **Depuis 2026-07-19, un lien `type:"acompte"` arme le DÉBIT AUTOMATIQUE du solde** (même rail que les résas du tunnel) : `customer_creation:always` + `payment_intent_data[setup_future_usage]:off_session` (carte enregistrée, Stripe affiche nativement le consentement + gère SCA) + metadata `pay_plan:"2x"` / `balance_amount` / `due_date` → `stripe-webhook.storePaymentSchedule()` insère dans `payment_schedule` → `charge-balance.js` prélève off-session à J-30. Garde-fou `canArmAutoDebit` (`src/utils/soldeSchedule.js`, 10 tests) : **n'arme QUE si le type est acompte, le solde > 0 et l'échéance strictement future** — un `due_date` vide passerait le filtre SQL `due_date <= today` (`"" <= date` = VRAI en SQLite) et déclencherait un débit IMMÉDIAT. ⚠️ **`payment_schedule.balance_amount` est en EUROS** (charge-balance fait `×100`) alors que les montants de devis sont en CENTIMES → `balanceEurosFromCents()` obligatoire (sinon débit ×100 : 4 012 € → 401 200 €). |
...
| `/api/devis-solde-cron` | GET | `devis-solde-cron.js` | Cron quotidien : gère le solde des devis en 2 fois (lien Stripe génération J-30, relances J-25/J-20, annulation auto J-15). Auth `?secret`. `?dry=1`. **Skip `debit_auto`** : si une ligne `payment_schedule` existe pour le même `bien_id`+`checkin` (devis armé en prélèvement automatique depuis 07/2026), ce cron ne fait RIEN — sinon on cumulerait deux encaissements du même solde (lien + débit off-session) et l'annulation J-15 tuerait un séjour dont le solde est en réalité programmé. **Anti-annulation silencieuse** : si l'email voyageur manque, escalade à l'hôte au lieu d'annuler un séjour dont l'acompte est déjà encaissé (bug vécu Cambier 07-19). |
...
| `/api/meta-ads-campaign` | GET/POST | `meta-ads-campaign.js` | Prépare/crée (Marketing API) les campagnes Meta Ads depuis `src/config/metaCampaignBrief.js` (source unique éditable — audiences/copy/budget). **Toujours `status:"PAUSED"`** — aucune dépense possible depuis cet endpoint ; l'activation reste un geste manuel de Vincent dans Ads Manager, jamais déclenché par ce code ni par Claude. GET ou POST sans `confirm:true` = aperçu seul (résout intérêts/régions, ne crée rien). POST `{campaign, confirm:true}` = création réelle (idempotente par nom). Carrousels multi-photos via `creative.images[]` (`child_attachments`) ; POST `{refreshCreatives:true}` remplace les créatives/annonces existantes (graphDelete ad→créative puis recrée). Auth Bearer admin uniquement. |
| `/api/meta-ads-insights` | GET | `meta-ads-insights.js` | **Agent budget pub, brique 1 (MESURE)** — lecture READ-ONLY des perfs des campagnes Meta (spend/impressions/clics/vues de page/conversions/ROAS) via Graph insights (`?window=7d\|30d`, `?level=campaign\|adset`). Logique pure `src/utils/metaAdsInsights.js`. Expose un bloc `health` (`no_spend`/`traffic_only`/`ok`) qui dit honnêtement si la mesure est exploitable — l'agent NE DOIT PAS arbitrer sur le ROAS tant que le tracking purchase ne remonte pas. Aucun `mutate`. Auth Bearer admin. |
| `/api/ad-budget-agent` | GET | `ad-budget-agent.js` | **Agent budget pub, brique 2 (DÉCISION, ADVISORY STRICT)** — croise le modèle CAC par bien (`src/utils/adBudgetAgent.js`, CAC plafond = 50 % de la commission Booking nette évitée, RM-08) avec les perfs Meta (`meta-ads-insights` niveau ad set) → **vision budget par bien** (`?budget=600` : combien dépenser par bien, seuls Amaryllis/Zandoli/Géko éligibles) + arbitrage scale/hold/cut. **Ne modifie AUCUN budget, ne dépense RIEN** — reco seulement. Auth Bearer admin. |
| `/api/attribution-survey` | GET/POST | `attribution-survey.js` | **NIVEAU 3 de la mesure pub — questionnaire post-achat.** POST public rate-limité (10/h/IP) : le voyageur répond « comment nous avez-vous connus ? » en 1 clic sur `/merci` (`src/Merci.jsx`), liste FERMÉE de sources (`SOURCES`) — une saisie libre produirait 40 orthographes du même canal ; `autre` porte un texte libre lu à la main, jamais agrégé. Index unique sur `booking_ref` → un rechargement de la page de confirmation ne pèse pas 2× dans la ventilation. GET admin/`?secret` : ventilation agrégée via `declaredAttribution` (`src/utils/adFinanceKpis.js`) — **marquée `exploitable:false` sous 20 réponses**, pour interdire de lire un pourcentage sur 3 réponses comme un signal. Raison d'être : les plateformes s'auto-attribuent leurs résultats et sont aveugles au bouche-à-oreille ; c'est la seule attribution qu'aucune plateforme n'a intérêt à biaiser, donc celle qui doit arbitrer la répartition du budget entre canaux. |
| `/api/social-insights` | GET | `social-insights.js` | **Social Growth Manager, brique 1 (MESURE)** — snapshot quotidien des abonnés par plateforme (série temporelle D1 `social_snapshots`, 1 ligne/plateforme/jour UTC, idempotent), là où `/api/meta-insights` ne fait qu'un pull live non stocké. FB (`fan_count`) + IG (`followers_count`) via `META_PAGE_TOKEN` ; YouTube (`subscriberCount` Data API v3) si `YOUTUBE_CHANNEL_ID`+clé configurés (accepte id `UC…` ou handle `@…`) ; **GBP en `pending_access`** tant que `GBP_API_ENABLED≠1` (accès API Business Profile non accordé — demande en cours). Logique pure `src/utils/socialGrowth.js` (21 tests) : croissance WoW/MoM/%, verdict vs cible `SOCIAL_GROWTH_TARGET_PCT` (défaut 5%/mois), `health` honnête par plateforme (`measurable`/`not_configured`/`pending_access`/`error`) — **ne persiste JAMAIS un chiffre non mesurable** (pas de faux zéro), ne totalise que le mesurable. `?dry=1` calcule sans écrire. Cron Worker 9h UTC (aux côtés de `ad-budget-execute`). Auth `?secret=POSTSTAY_SECRET` ou Bearer admin. |
| `/api/social-growth-agent` | GET | `social-growth-agent.js` | **Social Growth Manager, brique 2 (DÉCISION, ADVISORY STRICT)** — le « cerveau » du responsable réseaux. Croise la croissance abonnés (`social-insights`), l'engagement 30j (`meta-insights`) et la cadence éditoriale à venir (`editorial-calendar`) → synthèse LLM (`callLLM` tier smart, `json_object`) de recos organiques concrètes par plateforme, priorisées par l'écart à la cible. **Garde-fous honnêteté** (`src/utils/socialGrowthAgent.js`, 13 tests) : `sanitizeRecos` écarte toute action à tactique bannie (**achat d'abonnés / faux engagement / bots / follow-unfollow / boost payant** = décision de Vincent), borne à 4 recos × 3 actions, ne garde que les plateformes connues. **Ne publie rien directement, ne dépense rien** — aucune promesse magique d'abonnés (raisonne sur les chiffres réels). **Passerelle éditoriale (2026-07-22)** : au-delà des recos, l'agent émet un `content_plan` (1-2 posts concrets) → `planEditorialSlots` (validé, capé `SOCIAL_GROWTH_MAX_SCHEDULE` défaut 2, dédup sur créneaux libres J+3→J+17, écarte bien inconnu/format invalide/tactique bannie) → **insère des entrées `planned` dans `editorial_calendar`** qui suivent le pipeline existant (draft-gen → **gate qualité** → approbation → publication). **Jamais insérées `approved`/`published` directement — le gate reste le garde-fou.** Kill-switch `SOCIAL_GROWTH_SCHEDULE=0`. Persiste chaque analyse en D1 `social_growth_reports` (historique + source UI, inclut `content_scheduled`), push ntfy digest anti-fatigue (silence si 0 reco actionnable). Cron Worker lundi 6h UTC. `?dry=1` = analyse sans écrire ni notifier ; `?latest=1` = lit le dernier rapport stocké **sans appel LLM** (source de l'onglet 📈 Croissance audience, `CroissanceTab.jsx` — tuiles FB/IG/YouTube/GBP + verdict vs cible + recos). Auth `?secret=POSTSTAY_SECRET` ou Bearer admin. |
| `/api/social-impact` | GET | `social-impact.js` | **Social Growth Manager, boucle de FEEDBACK** — ferme mesure→décision→contenu→publication→**mesure** : pour chaque post `editorial_calendar` `published`, delta d'abonnés (FB+IG, D1 `social_snapshots`) des jours J+1..J+2 vs J-2..J-1 (jour J neutre), en absolu et %. Isole les posts issus de l'agent (marqueur `source:growth-agent` dans `brief`) via `growth_agent` pour juger SA perf vs la planif manuelle. Logique pure `src/utils/socialImpact.js` (8 tests, symétrique de `agentsImpact.js` mais sur les abonnés). Lu par `social-growth-agent` (biaise le `content_plan` vers formats/biens qui gagnent des abonnés) + affiché dans l'onglet 📈 Croissance audience. Se remplit ~1 point/jour depuis le déploiement du tracking (deltas fiables après ~30j). `?days=45` `?window=2`. Auth Bearer admin ou `?secret`. |
| `/api/ad-budget-execute` | GET | `ad-budget-execute.js` | **Agent budget pub, brique 3 (EXÉCUTION plafonnée, shadow→live)** — ferme la boucle : au lieu de recommander, AGIT dans un périmètre borné PAR CONSTRUCTION (pas juste la doc). 🟢 Autorisé (gestion libre hausse ET baisse) : augmente un ad set actif rentable de +20% ; **baisse graduelle -25%** d'un ad set au CAC au-dessus du plafond RM-08 (`decrease_budget`), qui décroît jour après jour jusqu'à la **pause** sous le plancher 3€/j (`planExecutionAction`). **PLAFOND GLOBAL DUR 600€/mois jamais dépassé** (`enforceGlobalMonthlyCap`, demande explicite Vincent) : la somme des budgets mensuels des ad sets actifs après décisions est bornée à `budgetMax` (défaut 600€) — seules les hausses sont rabotées, jamais les baisses. Logique pure `src/utils/adBudgetAgent.js` (22 tests). 🔴 Structurellement impossible : aucune fonction (ici ni dans `meta-ads-campaign.js`) ne sait activer un ad set en pause, créer une campagne ou dépasser le plafond — ce sont des dépenses NOUVELLES, elles restent le clic manuel de Vincent, même en mode `live` (règle absolue). Mode par `env.AD_AGENT_MODE` (défaut `shadow` = calcule/journalise sans écrire) ; `?dry=1` force shadow ; kill-switch `AD_AGENT_DISABLED=1`. Google Ads = lecture seule (aucune primitive d'écriture Google Ads n'existe), toute reco Google reste proposée seulement. Trace chaque décision en D1 `ad_budget_actions`. Cron Worker quotidien 9h UTC (aux côtés de `kpi-sentinel`). Auth `?secret=POSTSTAY_SECRET` ou Bearer admin. |
...
**`src/data/biens.js` = source unique des FAITS des 7 biens** (module pur, importable par les 3 runtimes : Functions, prerender Node, front Vite). Champs : `id/nom/type/prix/capacite/chambres/lieu/postal/coords/rating/reviews/bookable/photos/seoTitle/seoDesc` + helpers `ALL_BIENS`/`VILLAS`/`getBien`/`isMartinique`. Consommé par `functions/[slug].js` (meta+JSON-LD fiches), `scripts/prerender.mjs` (meta + `@graph` VacationRental via `buildRentalsGraph`), `functions/api/_biens.js` (faits + grounding agents), `PublicSite.jsx` (`canonFacts()` spread les faits ; le display riche reste local) et `scripts/generate-llms-txt.mjs` (génère `public/llms.txt` au build — filtre `bookable:true`, cf. GEO ci-dessous). Import depuis `src/` validé au bundling esbuild des Functions. Garde-fou : `src/__tests__/biens-consistency.test.js`. **Nomenclature : seuls Amaryllis & Iguana = « villas »** ; Iguana `bookable:false` (bail long).
...
### 8. GEO (visibilité IA) — Cloudflare peut bloquer les crawlers IA SANS QUE `robots.txt` LE MONTRE (trouvé 07/2026)

- `robots.txt` autorise tout (`Allow: /`) et référence `public/llms.txt` (généré au build par `scripts/generate-llms-txt.mjs` depuis `src/data/biens.js`, filtré `bookable:true` — ne JAMAIS l'éditer à la main, il est régénéré à chaque `npm run build`). **Mais un réglage Cloudflare au niveau du edge (Security → Bots, catégorie "AI Scrapers and Crawlers", ou une règle WAF) peut bloquer GPTBot/PerplexityBot/ClaudeBot/CCBot en 403 indépendamment de ce que dit `robots.txt`** — repéré uniquement en simulant les requêtes (`curl -A "<UA du bot>" https://villamaryllis.com/`), jamais visible en lisant le code ou `robots.txt`. Googlebot/Bingbot passent, les 5 signatures IA testées étaient bloquées. **Toujours vérifier par une requête réelle, pas en confiance sur la config déclarée** — c'est un réglage compte Cloudflare (dashboard), donc hors périmètre code, et une modification de sécurité que Claude ne doit jamais faire lui-même.
- Test de non-régression rapide : `curl -s -o /dev/null -w "%{http_code}" -A "Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko); compatible; GPTBot/1.1; +https://openai.com/gptbot" https://villamaryllis.com/` doit renvoyer `200`, pas `403`.
