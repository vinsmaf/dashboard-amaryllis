...
1. **SEO meta fiches = DOUBLE SOURCE** au niveau du *rendu* : `functions/[slug].js` (HTMLRewriter runtime) **écrase** `scripts/prerender.mjs`. La vérité de rendu = la fonction. Title ≤60c, desc ≤158c.
   ...
   - 🔴 **4ᵉ SOURCE NON MIGRÉE — `index.html` (lignes ~49-58)** : bloc JSON-LD VacationRental statique des 7 biens **codé en dur dans le `<head>`**, servi sur TOUTES les pages (c'est le structured-data principal vu par Google). **Données fausses** : iguana 4.75/**42** (réel 4 avis), nogent **4.95/42** (réel 4,8/18), coords obsolètes, « Schoelcher » sans ligature œ. À unifier (idéalement généré par prerender depuis le canonique en remplaçant le bloc `traf-010`). = **phase 3 / chantier suivant.**
