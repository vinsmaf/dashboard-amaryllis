...
1. **SEO meta fiches = DOUBLE SOURCE** au niveau du *rendu* : `functions/[slug].js` (HTMLRewriter runtime) **écrase** `scripts/prerender.mjs`. La vérité de rendu = la fonction. Title ≤60c, desc ≤158c.
   ...
   - ⏳ **Phase 2 (à faire)** : migrer `PublicSite.jsx` `const BIENS` (display riche : desc/amenities/photos/avis) + seeds `App.jsx` pour dériver du canonique → 8e bien = 1 seul endroit. Le bloc « liste des 7 biens » VacationRental de chaque page vient encore de PublicSite (cohérent avec le canonique, mais à unifier).
