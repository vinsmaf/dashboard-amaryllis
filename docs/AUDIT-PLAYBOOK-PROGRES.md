...
| **RM-15** 🔴 | ✅ FAIT (à déployer) | A/B tests mesuraient clic/scroll, pas la résa. Ajout `trackConversion(step:"purchase")` dans `Merci.jsx` à la résa confirmée, **uniquement pour les tests où l'user est exposé** (`listActiveVariants`, pas de fausse attribution). Vérifié fonctionnellement (event `ab_conversion` capturé). | à committer |
