...
- **Cause — PROUVÉE en prod le 2026-07-17** (`wrangler pages deployment tail`, message de la runtime Cloudflare elle-même) :
  > `waitUntil() tasks did not complete within the allowed time after invocation end and have been cancelled.`
  
  `context.waitUntil(refreshReadCache(...))` **ne peut pas aboutir** : l'action `read` dure **~32 s** (mesuré : 9 entités × 12 mois + 706 lignes de résas), au-delà du budget de tâche de fond post-réponse. **0 succès sur 6 tentatives observées.** Chaque lecture périmée relançait un refresh de 32 s aussitôt tué → cache jamais mis à jour → servi tel quel jusqu'au HARD_TTL de **24 h**, tout en consommant du quota Apps Script pour rien.
- **⚠️ Le piège dans le piège** : la tâche est **ANNULÉE**, pas en erreur → **aucune exception n'est levée** → le `.catch()` ajouté pour logger l'échec ne se déclenchait **jamais**. Le premier correctif (logger l'échec) était donc une **illusion de sécurité** : on croyait surveiller un mécanisme qui n'aboutissait jamais, et le log serait resté désespérément vide. C'est **l'absence de log** qui a mis sur la piste, pas sa présence. Le refresh en fond a fini par être **retiré** (il coûtait sans rien rapporter) ; `revenue-summary.js` garde le même pattern, mais son calcul est bien plus court — ne pas le retirer là-bas par analogie.
...
- **Garde-fou** : `functions/api/get-availability.test.js` fige les invariants — un résultat dégradé n'est jamais caché, et 2 feeds KO donnent `blockedDates:[]` **avec** `degraded:true` (≠ « tout est libre »).
- **Extension 2026-07-18 (2e mode de panne, dette pollinisation fermée)** : le `degraded` initial ne couvrait qu'un feed **MUET** (`!text` : injoignable / non-iCal). Un feed qui RÉPOND avec un VCALENDAR valide mais dont le **format des VEVENT a changé** (Airbnb modifie DTSTART/DTEND) passait encore : le parseur n'en extrait aucune nuit → « aucune date bloquée » = « tout libre » servi ET caché → même surbooking, mais invisible car le feed « répond » (200, VCALENDAR ok). Fix : `feedSuspect(text, nightsExtracted)` (`src/utils/icalHealth.js`, 9 tests) compare le **volume brut** (nombre de `BEGIN:VEVENT` présents) au **résultat** (nuits extraites) — `suspect = VEVENT présents ET 0 nuit`. Un feed SANS aucun VEVENT reste NON dégradé (bien légitimement sans résa sur ce canal, sinon on bloquerait la vente à tort). `degraded` élargi + `sources.{airbnb,booking}.suspect` exposé. Leçon appliquée (cross-learning) : un composant qui lit une source non contractuelle doit exposer une métrique de VOLUME BRUT distincte du résultat filtré, sinon « 0 résultat » est ambigu et l'ambiguïté se résout du côté rassurant. 2 tests d'intégration ajoutés (`get-availability.test.js` : format cassé → degraded / vide légitime → non dégradé). ⚠️ **Reste hors scope** : `booking-sync.mjs` (extranet) et parsing avis/concurrents non audités sous cet angle — seul le chemin de VENTE (`get-availability`) était prioritaire.
