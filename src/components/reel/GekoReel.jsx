// src/components/reel/GekoReel.jsx
// Reel cinématique Géko (42s, format 9:16) — même architecture RAF que VillaAmaryllisReel.
// Scènes : Hook → LowerThird → Features → Review → Price → CTA
...
const SHOTS = [
  { src: "/photos/geko/03.webp",  start: 0.0,  end: 8.0,  fromScale: 1.00, toScale: 1.14, fromX: -1.0, toX:  1.5, fromY:  0.0, toY: -0.8, dim: 0.38 },
  { src: "/photos/geko/08.webp",  start: 7.5,  end: 16.0, fromScale: 1.16, toScale: 1.00, fromX:  2.0, toX: -2.0, fromY:  0.5, toY:  0.0, dim: 0.22 },
  { src: "/photos/geko/15.webp",  start: 15.5, end: 23.5, fromScale: 1.00, toScale: 1.12, fromX:  0.0, toX:  0.0, fromY:  1.0, toY: -1.5, dim: 0.20 },
  { src: "/photos/geko/17.webp",  start: 23.0, end: 30.5, fromScale: 1.12, toScale: 1.00, fromX:  1.5, toX: -1.5, fromY:  0.0, toY:  1.0, dim: 0.22 },
  { src: "/photos/geko/22.webp",  start: 30.0, end: 37.0, fromScale: 1.00, toScale: 1.14, fromX: -1.5, toX:  2.0, fromY:  0.0, toY: -0.5, dim: 0.25 },
  { src: "/photos/geko/05.webp",  start: 36.5, end: 42.0, fromScale: 1.00, toScale: 1.08, fromX:  0.0, toX:  0.0, fromY:  0.0, toY: -1.0, dim: 0.32 },
];
...
function GekoTopChrome({ t, coral }) {
      ...
      <div className="rc-top-row">
        ...
        <div className="rc-eyebrow-wrap">
          <div className="rc-eyebrow-key">
            <span className="rc-eyebrow" style={{ color: coral }}>MARTINIQUE · SAINTE-LUCE</span>
          </div>
        </div>
      </div>
    </div>
...
function SceneHook({ t, coral, price }) {
  if (t > 7) return null;
  ...
  return (
    <div className="rc-scene">
      ...
      <div style={{
        ...
        fontFamily:"'Jost',sans-serif", fontWeight:200, fontSize:200, letterSpacing:"0.06em",
      ...
      <div style={{
        ...
        Piscine · T2 lumineux · Sainte-Luce
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          ✦ −15% vs Airbnb · Réservation directe
        </div>
      </div>
    </div>
  );
    ...
    <div className="rc-scene">
      ...
      <div style={{
        ...
        <div style={{ fontFamily:"'Jost',sans-serif", fontWeight:300, fontSize:22, letterSpacing:"0.42em", textTransform:"uppercase", color:"#c9a673", marginBottom:6 }}>
          Villa · Sainte-Luce, Martinique
        </div>
        <div style={{ fontFamily:"'Jost',sans-serif", fontWeight:200, fontSize:80, letterSpacing:"0.12em", textTransform:"uppercase", color:"#fff", lineHeight:1 }}>
          VILLA GÉKO
        </div>
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic", fontSize:34, color:"rgba(255,255,255,0.82)", marginTop:4 }}>
          Piscine privée · T2 lumineux · 4 voyageurs
        </div>
      </div>
      {/* Rating chip */}
      {rop > 0 && (
        <div style={{
          ...
          <span style={{color:"#c9a673",fontSize:36}}>★</span> 4,7 <span style={{opacity:0.5}}>·</span> 22 avis
        </div>
      )}
    </div>
  );
...
function SceneFeatures({ t, coral }) {
  if (t < 12 || t > 24) return null;
  const exit = t > 22.5 ? easeOutExpo(clamp01((t - 22.5) / 0.9)) : 0;
  const features = [
    { icon:"🏊", label:"Piscine", val:"Cascade privée", at:12.8 },
    { icon:"🌿", label:"T2", val:"Lumineux & zen", at:13.8 },
    { icon:"📶", label:"Wifi", val:"Starlink", at:14.8 },
    { icon:"🛏", label:"1 chambre", val:"+ séjour confort", at:15.8 },
    { icon:"🚗", label:"Parking", val:"Privé", at:16.8 },
    { icon:"✓", label:"Direct", val:"−15% Airbnb", at:17.8 },
  ];
  return (
    <div className="rc-scene">
      <div style={{ position:"absolute", inset:0, background:"linear-gradient(270deg, rgba(14,59,58,0.72) 0%, transparent 58%)" }}/>
      {features.map((f, i) => {
        const p = easeOutExpo(clamp01((t - f.at) / 0.55));
    ...
    <div className="rc-scene">
      ...
      <div style={{
        ...
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic", fontSize:30, lineHeight:1.5, color:"#0e3b3a" }}>
          "Joli T2 lumineux, parfait à deux, la piscine cascade est un vrai plus. Rapport qualité-prix imbattable. Hôte adorable et très réactif."
        </div>
      ...
      {rop > 0 && (
        <div style={{
          ...
          <div style={{ fontFamily:"'Jost',sans-serif", fontWeight:200, fontSize:180, color:"#fff", letterSpacing:"-0.02em", lineHeight:1, textShadow:"0 8px 40px rgba(0,0,0,0.5)" }}>4,7</div>
          <div style={{ fontFamily:"'Jost',sans-serif", fontWeight:300, fontSize:24, letterSpacing:"0.32em", textTransform:"uppercase", color:"#c9a673" }}>Note moyenne · 22 avis</div>
        </div>
      )}
    </div>
  );
...
function ScenePrice({ t, coral, price }) {
...
// ── Main component ────────────────────────────────────────────────────────────
export default function GekoReel({
  coral      = "#c47254",
  price      = "110",
  hook       = "Votre piscine cascade. Le jardin tropical. La Martinique à votre rythme.",
  ...
  // IntersectionObserver — stoppe RAF hors viewport
  useEffect(() => {
    const el = containerRef.current;
  ...
  if (reducedMotion) {
    return (
      <div className={`rc-page ${className}`} style={{ ...pageStyle, background:"var(--c-void)" }} aria-label="Géko">
        <img src={SHOTS[0].src} alt="Géko — Sainte-Luce, Martinique"
        ...
        <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:"32px 24px", fontFamily:"'Jost',sans-serif", color:"#faf5e9", textAlign:"center" }}>
          <div style={{ fontWeight:200, fontSize:28, letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:8 }}>VILLA GÉKO</div>
          <div style={{ fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic", fontSize:18, opacity:0.85 }}>Sainte-Luce · Martinique</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`rc-page ${className}`} style={pageStyle} aria-label="Reel Géko">
      ...
      {scale != null && (
        ...
        >
          ...
          <div className="rc-photo-bed">
            {SHOTS.map((shot, i) => (
              <img key={shot.src + i} ref={el => { imgRefs.current[i] = el; }} src={shot.src} alt=""
                className="rc-photo" style={{ opacity:0, transform:`translate(-50%,-50%) scale(${shot.fromScale}) translate(${shot.fromX}%,${shot.fromY}%)`, zIndex: i===0 ? 2 : 1 }} />
            ))}
            <div ref={dimRef} className="rc-video-dim" style={{ opacity: SHOTS[0].dim }} />
          ...
          {/* Overlay scenes */}
          <GekoTopChrome t={tState} coral={coral} />
          <SceneHook      t={tState} coral={coral} price={price} />
          <SceneLowerThird t={tState} />
          <SceneFeatures  t={tState} coral={coral} />
          <SceneReview    t={tState} />
          <ScenePrice     t={tState} coral={coral} price={price} />
          ...
          {!ready && (
            <div className="rc-loading">
              ...
              <div className="rc-loading-stack">
                ...
                <div className="rc-loading-eye">VILLA GÉKO</div>
