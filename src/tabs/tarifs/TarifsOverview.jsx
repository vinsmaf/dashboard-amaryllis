...
// ⚠️ Règle critique (cf. .memory/LEARNINGS.md 2026-06-07) :
// AUCUNE opération .filter()/.map()/.keys() sur un import d'App.jsx AU TOP-LEVEL.
// La circularité App.jsx → Tarifs.jsx → tarifs/* → App.jsx rend ces imports
// undefined au moment de l'évaluation du module. Tout doit être différé dans
// le composant (ou wrappé avec || []).
...
// Fallback hardcodé (utilisé UNIQUEMENT au top-level si CAL_BIEN_IDS pas encore évalué)
const FALLBACK_BIEN_IDS = ["amaryllis", "zandoli", "iguana", "geko", "mabouya", "schoelcher", "nogent"];

function priceColor(price, basePrice) {
  if (price === null) return "rgba(245,158,11,0.12)";
  const ratio = price / (basePrice || 100);
...
// Calcule le prix moyen d'un mois pour un bien donné. null = aucun prix saisi.
function monthAvg(daily, bienId, year, month, defaultPrix) {
...
export default function TarifsOverview() {
  ...
  // ⚠️ DANS le composant, pas au top-level — sinon CAL_BIEN_IDS peut être undefined
  const overviewBiens = useMemo(() => {
    const ids = CAL_BIEN_IDS || FALLBACK_BIEN_IDS;
    return ids.filter(id => id !== "iguana");
  }, []);

  const months = useMemo(() => next12Months(), []);
  const labels = BIEN_LABELS || {};
  const defaultPrix = DEFAULT_PRIX || {};
  const moisCal = MOIS_CAL || ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"];
  ...
  const gekoZandoliLinked = useMemo(() => {
    return months.every(({ year, month }) => {
      const g = monthAvg(daily, "geko", year, month, defaultPrix.geko || 150);
      const z = monthAvg(daily, "zandoli", year, month, defaultPrix.zandoli || 220);
      if (g === null && z === null) return true;
      if (g === null || z === null) return false;
      return Math.abs(g - z) / Math.max(g, z) < 0.05;
    });
  }, [daily, months, defaultPrix]);
          ...
          <tbody>
            {months.map(({ year, month }) => (
              <tr key={`${year}-${month}`} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                {overviewBiens.map(id => {
                  const base = defaultPrix[id] || 100;
                  const avg = monthAvg(daily, id, year, month, base);
                  return (
                    <td
                      key={id}
                      onClick={() => jumpToBien(id)}
                      title={`Cliquer pour éditer ${labels[id] || id} dans le calendrier`}
                      style={{
