...
describe("resolveLivePrice — même résolution que le site public", () => {
  it("priorise le prix journalier (override serveur) s'il existe", () => {
    expect(resolveLivePrice("zandoli", "2026-08-01", { zandoli: { "2026-08-01": 150 } })).toBe(150);
  });
  it("retombe sur le seed calibré (SEED_DAILY_PRICES) si pas d'override serveur", () => {
    // 2026-08-15 : Amaryllis a un prix seed calibré (~430€), pas le flat bien.prix (280€) —
    // c'est exactement le piège vécu 2026-07-18 (comparaison RM vs 280€ au lieu du vrai prix affiché).
    const live = resolveLivePrice("amaryllis", "2026-08-15", {});
    expect(live).not.toBe(280);
    expect(live).toBeGreaterThan(300);
  });
  it("l'override serveur gagne même quand un seed existe pour cette date", () => {
    const live = resolveLivePrice("amaryllis", "2026-08-15", { amaryllis: { "2026-08-15": 620 } });
    expect(live).toBe(620);
  });
  it("retombe sur bien.prix si ni override ni seed pour cette date", () => {
    expect(resolveLivePrice("zandoli", "1999-01-01", {})).toBe(110); // prix de base biens.js, hors couverture seed
  });
...
describe("computeSignificantGaps — isole les écarts qui comptent", () => {
  ...
  // Overrides explicites plutôt que dates couvertes par le seed — un prix live déterministe,
  // qui ne bouge pas quand Vincent recalibre SEED_DAILY_PRICES.
  const daily = { zandoli: { "2026-08-01": 110 }, geko: { "2026-08-02": 110 } };
  ...
  it("filtre un écart en dessous du seuil (bruit)", () => {
    ...
    expect(computeSignificantGaps(recos, daily, opts)).toEqual([]);
  });
  ...
  it("garde un écart significatif (hausse)", () => {
    ...
    const gaps = computeSignificantGaps(recos, daily, opts);
  ...
  it("trie par écart € absolu décroissant", () => {
    const recos = [
      ...
      { property_id: "geko",    date: "2026-08-02", recommended_price_cents: 20000, lead_time_days: 10 }, // geko live 110 → +90
    ];
    const gaps = computeSignificantGaps(recos, daily, opts);
  ...
  it("liste vide → tableau vide", () => {
    expect(computeSignificantGaps([], daily, opts)).toEqual([]);
    expect(computeSignificantGaps(null, daily, opts)).toEqual([]);
  });
});
