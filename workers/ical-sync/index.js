...
async function runEditorialAutoPublish(env) {
  ...
  try {
    ...
    const ntfyTopic = env.NTFY_TOPIC || "amaryllis-alertes-7r4k9";
    // Plafond de tentatives : au-delà d'un nombre d'ÉCHECS RÉELS, l'entrée passe `failed` pour ne
    // plus bloquer la file (l'auto-publish reprend TOUJOURS la plus ancienne entrée `approved`).
    ...
    for (const e of dueEntries) {
      ...
      // Compteur porté par le `result` de l'entrée, incrémenté à chaque tentative.
      let attempts = 0;
      ...
      let pubData = null;
      try {
        // ⚠️ AUCUN timeout client sur la publication. C'est une opération NON IDEMPOTENTE et LENTE
        // (upload images/vidéo + création post Meta FB+IG, 20-40s+). Un abort côté Worker coupe la
        // Function AVANT qu'elle marque le draft `published` en D1 → au run suivant l'entrée est
        // reprise et RE-publiée → DOUBLONS réels sur les réseaux (vécu 2026-07-22 : carrousel publié
        // 2× sur FB à cause d'un timeout 25s prématuré). On laisse la Function répondre (elle a son
        // propre timeout côté Pages), pour qu'elle persiste `published` avant qu'on retente.
